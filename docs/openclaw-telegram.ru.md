...
Проверьте, что первый ответ содержит `Grill Us v0.5.0 · Grill · Room` и язык комнаты. После этого можно писать в обычном порядке. OpenClaw передаёт идентичность отправителя, а Grill Us связывает её с постепенно дополняемым списком участников и ведёт журнал авторства.
