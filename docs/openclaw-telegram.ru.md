...
Примените рецепт [«Тихая запись»](../recipes/quiet-record.ru.md), если по запросу из этих фоновых событий нужен протокол с авторством. Историей комнаты и политикой памяти продолжает управлять OpenClaw.
...
Используй имена отправителей, которые уже передаёт OpenClaw. Помоги решить, что
и зачем мы делаем, на уровне Grill. Спрашивай о непосредственном опыте или
полномочиях, когда они нужны для маршрутизации. Сохраняй автора каждого утверждения,
разногласия и адресацию вопросов по именам. Мы можем отвечать в любом порядке.
```

Проверьте, что первый ответ содержит версию `Grill Us v0.6.0`, режим комнаты и язык. После этого можно писать в обычном порядке. OpenClaw передаёт идентичность отправителя, а Grill Us связывает её с постепенно дополняемым списком участников и ведёт журнал авторства.
