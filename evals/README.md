...
These cases test protocol invariants rather than exact wording. Run each prompt in a fresh session with Grill Us available. Continue until the agent produces either its next round or a final record, then score the behavior against [`cases.yaml`](cases.yaml).
